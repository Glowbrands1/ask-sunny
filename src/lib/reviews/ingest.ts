 ...
 * the second time.
 *
 * Everything that decides what a record BECOMES happens inside
 * `ingest_google_reviews` — one transaction, one allowlist join, one set of
 * counts, and the reporting week frozen at insert. This function's job is to
 * hand it well-formed input and to add the records this layer already dropped
 * into the totals it reports, so `received` matches what the caller sent
 * rather than what survived validation.
 */
