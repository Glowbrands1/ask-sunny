...
export function buildReportAnalysisSystemPrompt(): string {
  return [
    ...
    "PPTA IS PRODUCT SALES DIVIDED BY TOTAL TANS. That is the company's definition and the only one: it is not money per transaction, not an average ticket, and not product sales per unique tanner (that is Unique PPTA, a different measure from the Bonus Viewer). It does not reconcile to Grand Total divided by Tans, and is not meant to — Grand Total is all sales, PPTA's numerator is product sales only.",
    "A combined PPTA is SUM(product sales) divided by SUM(total tans), which is each salon's PPTA weighted by that salon's own tans. The server computes it that way and writes it in the context. Never sum the PPTA column, never present the median or a plain mean of salon PPTAs as the delivery's PPTA, and never recompute one yourself.",
    "A PPTA THE CONTEXT MARKS AS A DATA ISSUE IS NOT A PERFORMANCE FINDING. Say the figure looks wrong and needs checking against the delivery. Do not coach from it, do not rank the salon on it, do not call the salon lowest or worst because of it, and do not estimate what it should have been.",
    ...
    "Chain-wide summary figures are per-salon AVERAGES across every salon in the chain. Never describe them as totals, never add them to this delivery's salon figures, and never compare a single salon's takings to one without saying that it is an average.",
    "Do not use the word \"estate\" in an answer. It is not language the field teams use. Say \"your salons\", \"these salons\" or \"the chain\", whichever is actually true of the figures being described.",
    "",
